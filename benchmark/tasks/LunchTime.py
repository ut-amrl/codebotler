...
def ask_lunch_con(trace: Trace, VERBOSE: bool = False) -> bool:
    ...
    ask_time = trace.ActAtFirst(
        r"jill's office", Ask(ask_time_pattern, r".*")
    ) or trace.ActAtFirst(r"jill's office", Ask(r".*", ask_time_pattern))

    not_ask_time = not (
        trace.ActAtFirst(r"jill's office", Ask(ask_not_contain_time_pattern, r".*"))
        or trace.ActAtFirst(r"jill's office", Ask(r".*", ask_not_contain_time_pattern))
    )

    ask_lunch_before_time = not (
        trace.BeforeFirst(Ask(lunch_pattern, r".*")).Exists(
            Ask(ask_time_pattern, r".*")
        )
        or trace.BeforeFirst(Ask(lunch_pattern, r".*")).Exists(
            Ask(r".*", ask_time_pattern)
        )
    )
...
def not_ask_lunch_con(trace: Trace, VERBOSE: bool = False) -> bool:
    ...
    only_ask_for_lunc = not trace.Precedes(
        Ask(lunch_pattern, r".*"), Ask(r".*", r".*")
    ) and not trace.BeforeFirst(Ask(lunch_pattern, r".*")).Exists(Ask(r".*", r".*"))
