...
def generic_con(trace: Trace, VERBOSE: bool = False) -> bool:
    ...
    go_to_check = not trace.Exists(GoTo(r"lobby")) and trace.Exists(
        GoTo(r"zarko's office")
    )
    ask_at_zarko_cheese = trace.ActAtFirst(
        "zarko's office", Ask(r".*", contain_cheese)
    ) or trace.ActAtFirst("zarko's office", Ask(contain_cheese, r".*"))
    ask_at_zarko_bread = trace.ActAtFirst(
        "zarko's office", Ask(r".*", contain_bread)
    ) or trace.ActAtFirst("zarko's office", Ask(contain_bread, r".*"))
    say_at_start = trace.AfterFirst(GoTo(r"zarko's office")).ActAtFirst(
        "start_loc", Say(r".*")
    )
    ...
    if VERBOSE:
        ...
        print_debug(
            "ask_at_zarko_cheese", ask_at_zarko_cheese, [label_OWK, label_A, label_AL]
        )
        print_debug(
            "ask_at_zarko_bread", ask_at_zarko_bread, [label_OWK, label_A, label_AL]
        )
