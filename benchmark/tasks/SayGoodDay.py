...
state0 = (
    State()
    .addLocation("sam's lab")
    .addLocation("amy's lab")
    .addLocation("lobby")
...
def say_con(trace: Trace, agent_name: str, VERBOSE: bool = False) -> bool:
    ...
    check_before_say = (
        trace.AfterFirst(GoTo(f"{agent_name}'s lab"))
        .BeforeFirst(Say(pattern))
...
def generic_con(trace: Trace, VERBOSE: bool = False) -> bool:
    ...
    check_person_at_lab = trace.ActAtFirst(
        r"sam's lab", CheckEntity(r".*")
    ) and trace.ActAtFirst(r"amy's lab", CheckEntity(r".*"))
