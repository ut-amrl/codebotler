...
state0 = (
    ...
    .addLocation("supply room")
    .addLocation("meeting room")
    .addLocation("start_loc")
    .addObject("stapler", "supply room")
    .addObject("stapler", "supply room")
    .addObject("stapler", "supply room")
...
def manipulation_con(trace: Trace, room_nums: List[int], VERBOSE: bool = False) -> bool:
    ...
    for room_num in room_nums:
        new_trace = trace
        place_at_printer = new_trace.ActAtFirst(
            f"printer room {room_num}", Place("stapler")
        )
        while new_trace.AfterFirst(GoTo(f"printer room {room_num}")).Exists(
            GoTo(f"printer room {room_num}")
        ):
            new_trace = new_trace.AfterFirst(GoTo(f"printer room {room_num}"))
            place_at_printer = place_at_printer or new_trace.ActAtFirst(
                f"printer room {room_num}", Place("stapler")
            )
    ...
    if VERBOSE:
        ...
        print_debug(
            "not_pick_at_printer_room", not_pick_at_printer_room, [label_M, label_AL]
        )
...
def test0(trace_elements: List[TraceElement], VERBOSE: bool = False) -> bool:
    ...
    trace = Trace(trace_elements)
    generic = generic_con(trace, VERBOSE)
    manipulation = manipulation_con(
        trace, [2, 3], VERBOSE
    )  # no need to check not place at 1 because only 2 picks
