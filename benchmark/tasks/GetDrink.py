...
state0 = (
    ...
    .addObject("water", "kitchen")
    .addObject("tea", "kitchen")
    .addObject("lemonade", "kitchen")
    .addObject("drink", "kitchen")
...
def pick_place_con(trace: Trace, drink_type: str, VERBOSE: bool = False) -> bool:
    ...
    if VERBOSE:
        print_debug(
            f"pick_drink_at_kitchen drink type {drink_type}",
            pick_drink_at_kitchen,
            [label_M, label_AL],
        )
...
def ask_con(trace: Trace, drink_type: str, VERBOSE: bool = False) -> bool:
    ...
    ask_drink = trace.ActAtFirst(
        r"lobby", Ask(contain_drink, r".*")
    ) or trace.ActAtFirst(r"lobby", Ask(r".*", contain_drink))

    if VERBOSE:
        print_debug(
            f"ask_constraint drink type {drink_type}", ask_drink, [label_A, label_AL]
        )
...
def generic_con(trace: Trace, VERBOSE: bool = False) -> bool:
    ...
    goto_lobby_first = not trace.BeforeFirst(GoTo(r"lobby")).Exists(
    ...
    no_goto_start_between = (
        not trace.AfterFirst(GoTo(r"lobby"))
        .BeforeLast(GoTo(r"lobby"))
        .Exists(GoTo(r"start_loc"))
    )
    go_to_order_check = (
        goto_lobby_first
        and kitchen_after_lobby
        and lobby_after_kitchen
        and no_goto_start_between
    )
