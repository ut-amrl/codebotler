...
from misc.benchmark_utils import contain_words_and_exlude_words, contain_words, print_debug
...
def test0(trace_elements: List[TraceElement], VERBOSE: bool = False) -> bool: 
    ...
    ask_cheese = trace.Exists(Ask(r".*", contain_cheese)) or trace.Exists(Ask(contain_cheese, r".*"))
    ask_bread = trace.Exists(Ask(r".*", contain_bread)) or trace.Exists(Ask(contain_bread, r".*"))
