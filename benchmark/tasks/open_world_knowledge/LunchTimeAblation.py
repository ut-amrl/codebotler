...
from misc.benchmark_utils import contain_words, contain_words_and_exlude_words, print_debug
...
# construct state
state0 = State().addLocation("aiden's office").addLocation("andrew's office").addLocation("start_loc") \
        .addAgent(".*", "aiden's office", [r"yes", r".*"]) \
        .addRobotLocation("start_loc")

state1 = State().addLocation("aiden's office").addLocation("andrew's office").addLocation("start_loc") \
...
def test0(trace_elements : List[TraceElement], VERBOSE: bool = False) -> bool:
    ...
    ask = trace.Exists(Ask(r".*", ask_time_pattern)) or trace.Exists(Ask(ask_time_pattern, r".*"))
    not_ask = not(trace.Exists(Ask(r".*", ask_not_contain_time_pattern)) or trace.Exists(Ask(ask_not_contain_time_pattern, r".*")))
