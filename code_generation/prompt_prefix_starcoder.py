...
def task_program():
    ...
    arjun_response = ask("Arjun", "Are you ready to go?", ["Yes", "No"])
    go_to(start_loc)
    say("Arjun said: " + arjun_response)
...
def task_program():
    ...
    alice_response = ask("Alice", "How many staplers do you need?", ["1", "2", "3"])
    go_to("supply room")
    say("Alice needs " + str(alice_response) + " staplers")
...
# Go to the kitchen. Wait for someone to show up. When someone shows up, ask them to place the diet coke from the fridge in your basket. If they said yes, come back and tell me you have brought the diet coke back.
def task_program():
    start_loc = get_current_location()
    go_to("kitchen")
    while True:
        ...
        time.sleep(1)
    go_to(start_loc)
    say("I have brought your diet coke back.")
